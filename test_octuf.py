...
def main():
    ...
    model = OCTUF(sensing_rate=args.sensing_rate, LayerNo=args.layer_num)
    ...
    checkpoint = torch.load("%s/net_params_%d.pth" % (model_dir, 85), map_location=device)
    ...
    with torch.no_grad():
        ...
        for img_no in range(ImgNum):
            ...
            rec_SSIM = ssim(*match_shapes(X_rec * 255, Iorg.astype(np.float64)), data_range=255)
            ...
            pad_y = np.zeros((Img_rec_yuv.shape[0] - X_rec.shape[0], X_rec.shape[1]))
            pad_x = np.zeros((Img_rec_yuv.shape[0], Img_rec_yuv.shape[1] - X_rec.shape[1]))
            X_rec_padded = np.concatenate((X_rec, pad_y))
            X_rec_padded = np.concatenate((X_rec_padded, pad_x), axis=1)
            Img_rec_yuv[:, :, 0] = X_rec_padded * 255
...
#     max_shape = [max(shape1[0], shape2[0]), max(shape1[1], shape2[1])]

#     img1_padded = np.pad(img1, ((0, max_shape[0] - shape1[0]), (0, max_shape[1] - shape1[1])))
#     img2_padded = np.pad(img2, ((0, max_shape[0] - shape2[0]), (0, max_shape[1] - shape2[1])))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--block_size', type=int, default=16, help='block size (default: 32)')
